...
int check_file_name(const char *str, char *ext, char *err)
{
    ...
    if (len <= extlen|| (ft_strrchr(str, '/') && ft_strlen(ft_strrchr(str, '/') + 1) <= extlen))
...
{
    if (check_file_name(str, ".cub", "The file name should end with .cub") == -1)
        return -1;
    if (check_file_existed(ptr, str) == -1)
        return -1;
    //print_2_arr(ptr->file_content);
    return 0;
}
