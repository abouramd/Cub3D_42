...
int	map_check(t_pars *ptr, size_t x, size_t y, int *np)
{
	...
	if (!(ptr->map[y][x] == '0' || ptr->map[y][x] == 'N' || ptr->map[y][x] == 'S' || ptr->map[y][x] == 'E' || ptr->map[y][x] == 'W' || ptr->map[y][x] == 'D'))
...
int	run_check(t_pars *ptr)
{
	...
	while (ptr->map[x])
	{
		...
		while (ptr->map[x][y])
		{
			if ((ptr->map[x][y] == '0' || ptr->map[x][y] == 'D' || ptr->map[x][y] == 'E' || ptr->map[x][y] == 'N' || ptr->map[x][y] == 'S' || ptr->map[x][y] == 'W')
