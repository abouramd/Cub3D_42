...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/08/11 10:04:36 by abouramd          #+#    #+#             */
/*   Updated: 2023/08/11 10:04:37 by abouramd         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_global(t_pars *ptr, char *str)
{
	...
	if (!strncmp(&str[index], "C ", 2))
		return (get_color(&ptr->ceil, &str[index + 2], "C"));
	if (ptr->no && ptr->so && ptr->we && ptr->ea && ptr->ceil && ptr->floor)
		return (1);
...
int	check_content(t_pars *ptr)
{
	...
	if (!ptr->no || !ptr->so || !ptr->we || !ptr->ea \
		|| !ptr->ceil || !ptr->floor)
		return (put_err("the file content", "You miss some info!!"), -1);
	if (ptr->file_content[index] == NULL)
		return (put_err("the file content", "there is no map in the file!!"),
			-1);
