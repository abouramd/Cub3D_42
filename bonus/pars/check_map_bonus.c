...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/08/11 10:04:39 by abouramd          #+#    #+#             */
/*   Updated: 2023/08/11 10:04:40 by abouramd         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	run_check(t_pars *ptr, size_t x, size_t y)
{
	...
	while (ptr->map[x])
	{
		...
		while (ptr->map[x][y])
		{
			if ((ptr->map[x][y] == '0' || ptr->map[x][y] == 'D'
					|| ptr->map[x][y] == 'E' || ptr->map[x][y] == 'N'
					|| ptr->map[x][y] == 'S' || ptr->map[x][y] == 'W')
